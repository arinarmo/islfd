...
## Environments
Asignar un elemento de una lista a `NULL` lo elimina de la lista. Hacerlo en un environment crea una referencia a `NULL`.
```{r, mysize=TRUE, size='\\small'}
e$a <- NULL
ls(e)
```
...
- PCA (Principal Component Analysis): Dado un conjunto de datos con valores reales. $X \in \mathbb{R}^{p \times n}